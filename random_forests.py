...
def Trials(data, ns, trial_type=0, crit='gini', k=5):
    ...
    for n in ns:
        ...
        for i in range(k):
            ...
            if trial_type == 0:
                clf = RandomForestClassifier(20, criterion=crit, min_samples_leaf=n)
            else:  
                clf = RandomForestClassifier(20, criterion=crit, max_depth=n)
...
if __name__ == '__main__':
    ...
    a, a_err = Trials((train_x, train_y), range(1, 64, 4))
    print 'starting trial 2'
    b, b_err = Trials((train_x, train_y), range(1, 25, 2), 1)
